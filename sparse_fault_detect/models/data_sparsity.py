...
from sparse_fault_detect.constants import BUS_TO_RELAY_MAPPING, CURRENT_CHANNELS, VOLTAGE_CHANNELS
...
def apply_sparsity_transform(
    ...
    Returns:
        np.ndarray: Reshaped and potentially masked data of shape (n_samples, n_timesteps * n_features).
    """
    ...
    reshaped_data = windows.copy()
    logger.debug(f"Reshaped data to {reshaped_data.shape}.")

    # Apply various data sparlsity techniques based on config
    ...
    logger.debug(f"Reshaped data to {reshaped_data.shape} with applied data sparsity techniques.")

    # Log how much of the data is zeroed out
